...
// - Implements full PID in joint space with integral clamping to prevent windup
// - Angles are wrapped to [-pi, +pi] when computing errors to avoid discontinuities
...
public:
    ...
    : Node("pid_controller_node"), data_received_(false), integral_(Vector6d::Zero())
    {
        ...
        RCLCPP_INFO(this->get_logger(), "PID Controller started: publishing to /ur_effort_controller/commands");
    }
...
private:
    ...
    // wrap angle into [-pi, +pi] to compute shortest-path angular errors
    ...
    void control_loop()
    {
        ...
        // compute wrapped errors and derivative
        Vector6d error;
        Vector6d derivative;
        for (int i = 0; i < 6; ++i) {
