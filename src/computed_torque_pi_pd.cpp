...
// Node implementing a computed-torque controller with PI for position
// and PD for derivative action. The controller computes joint-space
// torques using the robot's inertia (D), Coriolis/centrifugal (C),
// and gravity (g) terms and publishes torque commands.
class ComputedTorquePIPDController : public rclcpp::Node
{
public:
    ComputedTorquePIPDController()
    : Node("computed_torque_pi_pd_node"), data_received_(false), integral_(Vector6d::Zero())
    {
        Vector6d Kp_diag, Ki_diag, Kd_diag;
        Kp_diag << 100.0, 100.0, 100.0, 60.0, 50.0, 35.0;
        Ki_diag << 2.0, 2.0, 2.0, 0.8, 0.6, 0.4;
        Kd_diag << 20.0, 20.0, 20.0, 8.0, 6.0, 5.0;
        ...
        subscription_ = this->create_subscription<sensor_msgs::msg::JointState>(
            "/joint_states", 10, std::bind(&ComputedTorquePIPDController::joint_state_callback, this, _1));

        target_angles_sub_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
            "/target_angles", 10, std::bind(&ComputedTorquePIPDController::target_angles_callback, this, _1));
        ...
        timer_ = this->create_wall_timer(
            ...
            std::bind(&ComputedTorquePIPDController::control_loop, this));

        RCLCPP_INFO(this->get_logger(), "Computed torque + PI+PD controller started.");
    }
...
private:
    ...
    {
        // Map incoming JointState message positions/velocities to the
        // controller's joint vector according to `joint_order_`.
    ...
    {
        // Compute the robot dynamics terms used by the computed-torque law.
        // D: inertia matrix, C: Coriolis/centrifugal matrix, g: gravity vector.
    ...
    void control_loop()
    {
        ...
        // Compute position error and derivative (velocity) error.
        Vector6d e;
        Vector6d de;
        for (int i = 0; i < 6; ++i) {
        ...
        // Control law in joint space (PI + PD): u = Kp*e + Ki*integral + Kd*de
        Vector6d u = Kp_ * e + Ki_ * integral_ + Kd_ * de;

        Matrix6d D, C;
        Vector6d g;
        robot_dynamics(q_, dq_, D, C, g);

        // Map the desired joint accelerations/efforts through dynamics to
        // compute feedforward torques: tau = D*u + C*dq + g
...
int main(int argc, char * argv[])
{
    ...
    auto node = std::make_shared<ComputedTorquePIPDController>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
