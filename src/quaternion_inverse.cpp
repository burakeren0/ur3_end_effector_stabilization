...
private:
    ...
    void control_loop()
    {
        // Eğer veriler henüz gelmediyse bekle
        if (!data_received_ || !initial_imu_captured_) return;

        // =================================================================
        // 1. HEDEF ROTASYONUN GÜVENLİ HESAPLANMASI (Matris -> Quaternion)
        // =================================================================
        // Roll ve Pitch eksenlerinin karışmaması için hedefi en güvenli yol olan
        // Dünya matris çarpımıyla bulup, IK için Quaternion'a çeviriyoruz.
        Eigen::Matrix3d R_base_EE_target = R_world_base_.transpose() * R_imu_initial_ * R_base_EE_init_;
        Eigen::Quaterniond q_target_base(R_base_EE_target);

        // =================================================================
        // 2. İLERİ KİNEMATİK İLE ŞU ANKİ YÖNELİMİ BULMA
        // =================================================================
        ...
        Eigen::Matrix4d T4 = get_dh_matrix(q_(3), a_[3], d_[3], alpha_[3]);
        Eigen::Matrix4d T5 = get_dh_matrix(q_(4), a_[4], d_[4], alpha_[4]);
        Eigen::Matrix4d T6 = get_dh_matrix(q_(5), a_[5], d_[5], alpha_[5]);
        
        Eigen::Matrix4d T_j4 = T1 * T2 * T3;
        Eigen::Matrix4d T_j5 = T_j4 * T4;
        Eigen::Matrix4d T_j6 = T_j5 * T5;
        Eigen::Matrix4d T_all = T_j6 * T6;

        Eigen::Quaterniond q_curr_base(T_all.block<3,3>(0,0));

        // =================================================================
        // 3. QUATERNION HATASI VE JACOBIAN İLE TERS KİNEMATİK
        // =================================================================
        Eigen::Quaterniond q_err = q_target_base * q_curr_base.inverse();
        
        // Quaternion Hemisphere (Yarım Küre) Koruması - Ani taklaları engeller
        if (q_err.w() < 0.0) {
            q_err.coeffs() *= -1.0; 
        }
        
        Eigen::AngleAxisd aa_err(q_err);
        Eigen::Vector3d error_omega = aa_err.axis() * aa_err.angle();

        // Jacobian Matrisi
        Eigen::Vector3d z4 = T_j4.block<3,1>(0,2);
        Eigen::Vector3d z5 = T_j5.block<3,1>(0,2);
        Eigen::Vector3d z6 = T_j6.block<3,1>(0,2);

        Eigen::Matrix3d J_omega;
        J_omega << z4, z5, z6;

        // DLS (Singularity Koruması)
        double lambda = 0.1; 
        Eigen::Matrix3d J_T = J_omega.transpose();
        Eigen::Matrix3d I3 = Eigen::Matrix3d::Identity();
        Eigen::Matrix3d J_inv = J_T * (J_omega * J_T + lambda * lambda * I3).inverse();

        Eigen::Vector3d delta_q = J_inv * error_omega;

        // Yumuşak Adım (Yumuşak İntegrasyon)
        double K_ik = 1.5;            
        double dt = 0.01;             
        double max_vel = 1.0; // Saniyede en fazla ~57 derece dönebilir (Işınlanmayı önler)
        
        for(int i = 0; i < 3; i++) {
            double joint_vel = delta_q(i) * K_ik;
            joint_vel = std::clamp(joint_vel, -max_vel, max_vel);
            q_d_(3 + i) += joint_vel * dt;
        }

        // Sarmal (Wrap-around) Koruması
        q_d_(3) = shortest_angular_distance(q_(3), q_d_(3));
        q_d_(4) = shortest_angular_distance(q_(4), q_d_(4));
        q_d_(5) = shortest_angular_distance(q_(5), q_d_(5));


        // =================================================================
        // 4. TORK HESAPLAMA VE YAYINLAMA KISMI (TİTREŞİM ÇÖZÜMÜ)
        // =================================================================
        ...
        // --- KRİTİK DEĞİŞİKLİK: TİTREŞİMİ (JITTER) YOK ETMEK ---
        // Kv_direct 10.0'dı, bu yüzden hız gürültüsü motorları titretiyordu. 0.5'e çektik!
        Matrix6d Kp_direct = Matrix6d::Zero();
        Matrix6d Kv_direct = Matrix6d::Zero();
        
        Kp_direct.diagonal() << 0.0, 0.0, 0.0, 15.0, 15.0, 15.0; 
        Kv_direct.diagonal() << 0.0, 0.0, 0.0, 0.5, 0.5, 0.5;

        Vector6d tau = D * u + C * dq_ + g + (Kp_direct * e) + (Kv_direct * de);
