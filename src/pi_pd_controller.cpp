...
public:
    PIPDController()
    : Node("pi_pd_controller_node"), data_received_(false), integral_(Vector6d::Zero())
    {
        ...
        subscription_ = this->create_subscription<sensor_msgs::msg::JointState>(
            "/joint_states", 10, std::bind(&PIPDController::joint_state_callback, this, _1));

        target_angles_sub_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
            "/target_angles", 10, std::bind(&PIPDController::target_angles_callback, this, _1));
...
int main(int argc, char * argv[])
{
    ...
    auto node = std::make_shared<PIPDController>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
