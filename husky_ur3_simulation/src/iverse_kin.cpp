...
class IverseKinNode : public rclcpp::Node
{
public:
  IverseKinNode()
  : Node("iverse_kin")
  {
    ...
    target_pose_sub_ = this->create_subscription<std_msgs::msg::Float64MultiArray>(
      target_pose_topic_, 10, std::bind(&IverseKinNode::target_pose_callback, this, _1));
    target_angles_pub_ = this->create_publisher<std_msgs::msg::Float64MultiArray>(
      target_angles_topic_, 10);

    RCLCPP_INFO(
      this->get_logger(),
      "Iverse IK node started. Subscribing: %s, publishing: %s",
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<IverseKinNode>());
  rclcpp::shutdown();
  return 0;
}
