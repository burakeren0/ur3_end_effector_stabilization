...
# Launch file for the parkour UR3 scenario using a PI+PD joint controller.
# The file starts the simulation, then starts pose stabilization, inverse kinematics,
# and finally the PI+PD controller after the world is ready.
# This keeps the node startup predictable for the full controller chain.
...
def launch_setup(context, *args, **kwargs):
    ...
    delayed_pi_pd_controller = TimerAction(
        period=20.0,
        actions=[
            Node(
                package="ur3_end_effector_stabilization",
                executable="pi_pd_controller_node",
                output="screen",
            )
        ],
    )
...
def generate_launch_description():
    ...
    declared_arguments.append(  # Declare the CSV output path passed to the logger.
        DeclareLaunchArgument(  # Create the logger output-file launch option.
            ...
            default_value="/home/taylan/ur3_ws/src/ur3_end_effector_stabilization/analysis/end_effector_pose_pi_pd_log.csv",  # Keep PI+PD samples in a controller-specific CSV.
