...
# Launch file for the parkour UR3 scenario with computed torque + PID controller.
# It starts the simulation, spawns the robot, and launches target pose,
# inverse kinematics, and the PID controller in sequence.
# The launch file also handles world resolution and controller configuration.
...
def launch_setup(context, *args, **kwargs):
    ...
    delayed_computed_torque_pid = TimerAction(
        period=20.0,
        actions=[
            Node(
                package="ur3_end_effector_stabilization",
                executable="computed_torque_pid_node",
                output="screen",
            )
        ],
    )
...
def generate_launch_description():
    ...
    declared_arguments.append(  # Declare the CSV output path passed to the logger.
        DeclareLaunchArgument(  # Create the logger output-file launch option.
            ...
            default_value="/home/taylan/ur3_ws/src/ur3_end_effector_stabilization/analysis/end_effector_pose_ct_pid_log.csv",  # Keep computed-torque PID samples in a controller-specific CSV.
