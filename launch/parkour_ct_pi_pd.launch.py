...
# Launch file for the parkour UR3 scenario with computed torque + PI+PD controller.
# It initializes Gazebo, spawns the robot, and schedules the full control stack
# including pose stabilization and inverse kinematics.
# Delayed startup ensures that each component is launched in a stable order.
...
def launch_setup(context, *args, **kwargs):
    ...
    delayed_computed_torque_pi_pd = TimerAction(
        period=20.0,
        actions=[
            Node(
                package="ur3_end_effector_stabilization",
                executable="computed_torque_pi_pd_node",
                output="screen",
            )
        ],
    )
...
def generate_launch_description():
    ...
    declared_arguments.append(  # Declare the CSV output path passed to the logger.
        DeclareLaunchArgument(  # Create the logger output-file launch option.
            ...
            default_value="/home/taylan/ur3_ws/src/ur3_end_effector_stabilization/analysis/end_effector_pose_ct_pi_pd_log.csv",  # Keep computed-torque PI+PD samples in a controller-specific CSV.
